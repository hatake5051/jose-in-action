...
/**
 * [仕様] RFC7517#4 で記述してある JWK が持つ共通パラメータを表す。
 * [実装] K を指定すれば kty パラメータをその型に制限できる。
 */
type CommonJWKParams<K extends Kty = Kty> = {
   ...
   * 値 "sig" が署名のために使うことを、 "enc" が暗号化(or key agreement)のために使うことを表現する。
   */
  use?: KeyUse;
  /**
   * RFC7517#4.3
   ...
   * 参照先リソースは PEM フォーマットで表現され、最初の証明書はこの JWK が表現する他の公開鍵パラメータと一致しなければならない。
   *
   */
  x5u?: string;
  /**
